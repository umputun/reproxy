...
func TestFile_List(t *testing.T) {
	...
	// sorted by server name length, auth.example.com comes first (same length as srv.example.com but alphabetically first)
	assert.Equal(t, "^/api/(.*)", res[0].SrcMatch.String())
	assert.Equal(t, "http://127.0.0.4:8080/$1", res[0].Dst)
	assert.Empty(t, res[0].PingURL)
	assert.Equal(t, "auth.example.com", res[0].Server)
	...
	assert.Equal(t, "^/api/svc2/(.*)", res[1].SrcMatch.String())
	assert.Equal(t, "http://127.0.0.2:8080/blah2/$1/abc", res[1].Dst)
	assert.Empty(t, res[1].PingURL)
	assert.Equal(t, "srv.example.com", res[1].Server)
	...
	assert.Equal(t, "/api/svc3/xyz", res[3].SrcMatch.String())
	assert.Equal(t, "http://127.0.0.3:8080/blah3/xyz", res[3].Dst)
	assert.Equal(t, "http://127.0.0.3:8080/ping", res[3].PingURL)
	assert.Equal(t, "*", res[3].Server)
	assert.Equal(t, discovery.MTProxy, res[3].MatchType)
	assert.Nil(t, res[3].KeepHost)
	assert.Equal(t, []string{}, res[3].OnlyFromIPs)

	assert.Equal(t, "/web/", res[4].SrcMatch.String())
	assert.Equal(t, "/var/web", res[4].Dst)
	assert.Empty(t, res[4].PingURL)
	assert.Equal(t, "*", res[4].Server)
	assert.Equal(t, discovery.MTStatic, res[4].MatchType)
	assert.False(t, res[4].AssetsSPA)
	assert.Equal(t, []string{"192.168.1.0/24", "124.0.0.1"}, res[4].OnlyFromIPs)
	assert.True(t, *res[4].KeepHost)

	assert.Equal(t, "/web2/", res[5].SrcMatch.String())
	assert.Equal(t, "/var/web2", res[5].Dst)
	assert.Empty(t, res[5].PingURL)
	assert.Equal(t, "*", res[5].Server)
	assert.Equal(t, discovery.MTStatic, res[5].MatchType)
	assert.True(t, res[5].AssetsSPA)
	assert.Empty(t, res[5].OnlyFromIPs)
	assert.False(t, *res[5].KeepHost)
}
